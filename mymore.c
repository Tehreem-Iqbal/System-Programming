...
void more(FILE *in, char *const filename){ 
 ...
 /*void vim(char *const filename){
	pid_t pid;
	char *const parmList[] = {"/usr/bin/vi", filename, NULL};
	pid = fork();
	if (pid == -1){
		perror("fork error");
		exit(1);
        	...
        	execv("/usr/bin/vi", parmList);	
        }
        else if (pid > 0) { 
       	int wstatus; 
       	pid_t waitpid(pid_t pid_of_child, int *status, int options);
       	waitpid(pid, &wstatus, pid);//, WUNTRACED | WCONTINUED);
       }
