/*
This version should be able to redirect stdin and stdout for
the new processes by using < and > operators. It should also
add the functionality of the pipe operator (|) as well.
*/
...
void input_redirection(char **arglist){
	...
	while(arglist[i]!= NULL){
		if( !strcmp(arglist[i],"1>") ||!strcmp(arglist[i],">") || !strcmp(arglist[i],">>")){
			int fd_out;
			dp_out = dup(1); 
			if( !strcmp(arglist[i],">>"))
				fd_out = open(arglist[i+1], O_WRONLY  | O_APPEND , 0666); // Something is wrongs
			else 
				fd_out = open(arglist[i+1], O_WRONLY | O_CREAT , 0666);
			dup2(fd_out,1);
			i++;
		}
